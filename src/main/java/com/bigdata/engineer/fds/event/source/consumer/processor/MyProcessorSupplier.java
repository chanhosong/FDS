                ...
                try {
                    for (String word : words) {
                        ...
                        if (word.contains(EventConstants.NEW_ACCOUNT_EVENT_LOG_APPENDER.toLowerCase().trim())) {
                            ...
                            Map<String, LogEvent> d = new HashMap<>();
                            d.put(withdrawEvent.getAccountid(), withdrawEvent);
                            this.WithdrawEventStore.put(withdrawEvent.getCustomerid(), d);
                            ...
                            Map<String, LogEvent> d = new HashMap<>();
                            d.put(transferEvent.getTransferaccount(), transferEvent);
                            this.TransferEventStore.put(transferEvent.getCustomerid(), d);
                        }
                    }
                } catch (IOException e) {
                    e.printStackTrace();
